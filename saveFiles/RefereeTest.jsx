...
const RefereeTest = () => {
  ...
  const handleFinish = async () => {
    ...
    const passScore = QUESTION_CATEGORIES[category].passScore;
      ...
      <div
        className={`referee-test referee-test--result ${
          result.passed
            ...
            : 'referee-test--result-failed'
        }`}
      >
        <h2>
          {result.passed ? 'Tests ir NOKĀRTOTS ✅' : 'Tests NAV nokārtots ❌'}
        </h2>

        <p>
          Pareizas atbildes:{' '}
          <strong>
            {result.correctCount} no {result.total}
          </strong>
        </p>
        <p>
          Minimālais nepieciešamais rezultāts kategorijai {category}:{' '}
          <strong>{result.passScore}</strong> pareizas atbildes.
        </p>

        {/* Кнопка всё ещё возвращает на intro, но там старт будет заблокирован,
            т.к. availability.isOpen = false и reason = 'no_attempts_left' */}
        <button
          className="referee-test__start-btn"
